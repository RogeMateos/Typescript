...
export const PageInteractions = {
  ...
  toggleSection(sectionId) {
    const section = DOMUtils.getElement(`list-${sectionId}`);
    const icon = DOMUtils.getElement(`icon-${sectionId}`);

    if (section) {
      DOMUtils.toggleClass(section, "hidden");

      // Rotate icon
      if (icon) {
        if (section.classList.contains("hidden")) {
          DOMUtils.removeClass(icon, "rotate-180");
        } else {
          DOMUtils.addClass(icon, "rotate-180");
        }
      }
    }
  },
...
window.toggleSection = PageInteractions.toggleSection;
